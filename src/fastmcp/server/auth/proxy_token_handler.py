...
class ProxyTokenHandler(TokenHandler):
    ...
    async def handle(self, request: Request):  # noqa: C901 (keep parity with upstream)
        ...
        logger.info("ProxyTokenHandler invoked for /token request from %s", request.client)
        ...
        try:
            ...
            for k, v in parsed_qs.items():
                ...
                if k in {"client_secret", "code", "refresh_token"}:
                    ...
                    redacted[k] = value
            logger.info("/token request form from client: %s", redacted)
        ...
        try:
            ...
            logger.info("/token success response to client: %s", redacted_tokens)
        except Exception:  # noqa: BLE001
